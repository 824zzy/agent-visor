...
enum ToolEventProcessor {
    nonisolated private static let logger = Logger(subsystem: AppBranding.loggerSubsystem, category: "ToolEvents")
    ...
    /// Extract tool input from AnyCodable dictionary. Internal access so
    /// SessionStore's PermissionRequest path can share this conversion
    /// instead of hardcoding an empty input — AskUserQuestion's `questions`
    /// array would otherwise be lost at the placeholder-creation site.
    nonisolated static func extractToolInput(from hookInput: [String: AnyCodable]?) -> [String: String] {
        ...
        for (key, value) in hookInput {
            if let str = value.value as? String {
                ...
                // Preserve arrays/nested dicts (e.g. AskUserQuestion's
                // `questions`) as JSON strings so renderers can decode
                // them on demand. Without this, complex inputs vanish.
