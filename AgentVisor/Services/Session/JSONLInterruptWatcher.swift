...
private let logger = Logger(subsystem: AppBranding.loggerSubsystem, category: "Interrupt")
...
class JSONLInterruptWatcher {
    ...
    private let queue = DispatchQueue(
        label: AppBranding.loggerSubsystem + ".interruptwatcher",
        qos: .userInteractive
    )
    ...
    private func startWatching() {
        ...
        guard FileManager.default.fileExists(atPath: filePath) else {
            logger.warning("File does not exist: \(self.filePath, privacy: .public)")
            return
        }

        let handle: FileHandle
        do {
            handle = try FileHandle(forReadingFrom: URL(fileURLWithPath: filePath))
        } catch {
            let ns = error as NSError
            // NSCocoaErrorDomain code 257 = NSFileReadNoPermissionError, the silent
            // deny we get on Sequoia when agent-visor lacks Full Disk Access and the
            // JSONL was written by claude-code (com.apple.provenance xattr). Logging
            // domain+code+description so the failure is one-shot diagnosable.
            logger.error("Failed to open file: \(self.filePath, privacy: .public) — \(ns.localizedDescription, privacy: .public) (domain=\(ns.domain, privacy: .public), code=\(ns.code))")
            let openedPath = self.filePath
            Task { @MainActor in
                FullDiskAccessChecker.reportOpenFailure(error: error, path: openedPath)
            }
            return
        }
