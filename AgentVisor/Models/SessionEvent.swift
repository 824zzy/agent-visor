...
enum SessionEvent: Sendable {
    ...
    /// JSONL file was extended (append detected via DispatchSource).
    /// Triggers a debounced incremental parse without requiring a hook event.
    case fileExtended(sessionId: String, cwd: String)
    ...
    /// Agent Visor created a Codex thread through its own app-server.
    case codexAppServerThreadStarted(sessionId: String, cwd: String)

    /// Agent Visor forked a new claude-code session under a headless PTY
    /// (SpawnedSessionManager). Pre-seeds the row so it can be selected
    /// immediately; the real SessionStart hook reconciles in place.
    case visorSpawnedSessionStarted(sessionId: String, cwd: String)

    /// Backfill an assistant text block scraped from the terminal AX
    /// while JSONL is still buffering it. `boundToolUseId` is the
    /// AskUserQuestion's tool id; the synthetic item is removed when
    /// that tool's real JSONL entry arrives.
    case injectScrapedAssistantText(sessionId: String, boundToolUseId: String, text: String)

    /// Assistant text streamed from Codex app-server before the rollout
    /// JSONL watcher has replayed the same item.
    case codexAssistantDelta(sessionId: String, itemId: String, delta: String)

    /// User message from Codex app-server `turn/started`, mirrored before
    /// the rollout JSONL watcher has replayed the same item.
    case codexUserMessage(sessionId: String, itemId: String, text: String, images: [ChatImageAttachment])
}
...
extension HookEvent {
    nonisolated var isTerminalLifecycleStatus: Bool {
    ...
    nonisolated func determinePhase() -> SessionPhase {
        ...
        case .waitingForApproval:
            // Reached by observe-only agents (codex) whose hook reports the
            // status but doesn't set `expectsResponse` (claude-code takes the
            // branch above with full permissionSuggestions). We still want
            // the phase so the pill/sidebar shows "awaiting approval".
            return .waitingForApproval(PermissionContext(
                toolUseId: toolUseId ?? "",
...
extension SessionEvent: CustomStringConvertible {
    nonisolated var description: String {
        ...
        case .fileExtended(let sessionId, _):
            return "fileExtended(session: \(sessionId.prefix(8)))"
        ...
        case .codexAppServerThreadStarted(let sessionId, let cwd):
            return "codexAppServerThreadStarted(session: \(sessionId.prefix(8)), cwd: \(cwd))"
        case .visorSpawnedSessionStarted(let sessionId, let cwd):
            return "visorSpawnedSessionStarted(session: \(sessionId.prefix(8)), cwd: \(cwd))"
        ...
        case .injectScrapedAssistantText(let sessionId, let boundToolUseId, let text):
            return "injectScrapedAssistantText(session: \(sessionId.prefix(8)), tool: \(boundToolUseId.prefix(12)), chars: \(text.count))"
        case .codexAssistantDelta(let sessionId, let itemId, let delta):
            return "codexAssistantDelta(session: \(sessionId.prefix(8)), item: \(itemId.prefix(12)), chars: \(delta.count))"
        case .codexUserMessage(let sessionId, let itemId, let text, let images):
            return "codexUserMessage(session: \(sessionId.prefix(8)), item: \(itemId.prefix(12)), chars: \(text.count), images: \(images.count))"
        }
    }
}
