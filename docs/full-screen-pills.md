...
- The center notch indicator renders only on a display with a physical notch, where it follows the same visibility decision as the pills. It is decoration: it does not hit-test, and no global monitor turns clicks over it into window summons. Displays without a physical notch show no synthetic notch; their pills consolidate at center and reveal/hide as one group.
