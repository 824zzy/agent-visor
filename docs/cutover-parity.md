...
| Menu pills | Released active-first, 42-hour observed recent shortcuts, capsules, colors, actions, keycaps, hover inspectors, and attention order | Source-backed History is restored as dimmed recent candidates; `+N` counts every omitted navigator row; the `Pill Settings...` menu remains absent | Partial |
